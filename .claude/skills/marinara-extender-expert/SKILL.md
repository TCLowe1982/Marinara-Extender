...
- **"How do I see what it remembers?"** The **memory browser** — `http://127.0.0.1:3001/` (also `/memory`). Self-contained page served by the sidecar itself, so it works with no extension, no package, and whatever Engine version is installed. **Four ways in, none of which require knowing the URL** (`c2wd`): the startup banner prints it, `/setup` has an *Open memory browser* button, the launcher console takes **`M`**, and the page's own header links out to `setup` / `prompts` and back. There is **no way in from inside the Engine** — that is `hwlj`'s Option A (a package `client` entrypoint in the `chat-settings` slot), which needs a `ui` permission the manifest does not declare and an audit conversation TC has deferred. Browse by scope and character, click a memory to read its body, **Why?** on each memory reports how it fared in recent turns in plain language (injected with the reason, held back with the reason, or never considered). Edit / Delete / Restore live on the memory itself; **delete is soft** — it moves to *Recently deleted* and can be restored. **Purge** is the only irreversible action and is reachable only from inside the deleted view.
