...
2. **Retrieval invisibility** — it *is* stored but can't surface. **Largely fixed 2026-08-04 (`tp5`)**: ranking now also matches `IndexEntry.bodyTerms`, the names harvested from an entry's body, so a subject named only in a body is reachable. Measured on the Erica corpus, reachability went 43% → 84%. Two things still cause it, and both are worth asking about before assuming a capture gap:
   - **The store was never backfilled.** `bodyTerms` is written at entry creation; everything captured before the fix needs `node scripts/backfill-body-terms.mjs` (idempotent, safe to re-run). A row with no `bodyTerms` means *not yet harvested*, never *this entry has no names*.
   - **The name is never capitalised in the body.** Harvesting is name-shaped, so a lowercase mention stays invisible. That is `76aw` (entity/alias coreference), not something a wider term list fixes.

   Still distinguish from a gap by grepping **bodies** as well as summaries.
...
- **"How do I see what it remembers?"** The **memory browser** — `http://127.0.0.1:3001/` (also `/memory`). Self-contained page served by the sidecar itself, so it works with no extension, no package, and whatever Engine version is installed. **Four ways in, none of which require knowing the URL** (`c2wd`): the startup banner prints it, `/setup` has an *Open memory browser* button, the launcher console takes **`M`**, and the page's own header links out to `setup` / `prompts` and back. There is **no way in from inside the Engine** — that is `hwlj`'s Option A, and as of 2026-09-07 **nothing gates it**: the target slot is now `home-browser-tab` (not `chat-settings`), the `ui` permission exists and is declarable, and the mount point is live in the **installed** Engine v2.4.4. See `references/capability-package.md`, including its warning about which Engine copy to read. Browse by scope and character, click a memory to read its body, **Why?** on each memory reports how it fared in recent turns in plain language (injected with the reason, held back with the reason, or never considered). Edit / Delete / Restore live on the memory itself; **delete is soft** — it moves to *Recently deleted* and can be restored. **Purge** is the only irreversible action and is reachable only from inside the deleted view. **Three retirements, three surfaces, deliberately not merged:** *Recently deleted* (the user did it), *Discarded by the system* (`ud30` — the sidecar retired it on its own behalf, shown with the retiring reason **verbatim**, restore only and no purge), and supersession (rollback). The sidebar also carries **Needs you → Held for review** (`4z0h`) for re-rolls whose retirement did not settle the matter; at zero it reads `0 held — N discarded memories examined`, because the count that has to be *earned* is the denominator, not the numerator (`385b`).
- **"It's not remembering / nothing shows up."** First check the obvious chain: is the **sidecar running** (port 3001), is the **loader installed** in Marinara, did the extension **fetch the live file** (reload Marinara), are the **two constant lorebook entries** present? The panel button only appears if the sidecar is reachable. The memory browser is the fastest way to tell a capture gap from a retrieval miss: if the memory is listed, it was captured, and **Why?** says what happened to it.
