...
> - **The lane has a UI** (`4z0h`), at `/memory` as its **own sidebar entry** ("Needs you") — deliberately *not* under *Recently deleted*, which would reassert the conflation `discardedAt` exists to prevent. **It used to hide itself entirely when empty — that was reversed in `385b`, see the next bullet.** `GET /api/held` (returns `{ held, examined }`, filtered to `discarded-swipe`; the same file also carries the curator's withheld verdicts, a different lane with different actions) and `POST /api/held/resolve` (`dismiss` | `restore-entry` | `restore-displaced`), which checks the record is outstanding **first**, then restores, then stamps — a failed restore leaves it in the lane, and a stale tab gets "already settled" instead of a true-but-irrelevant error.
> - **The lane does not hide, and it prints its own denominator** (`385b`, 2026-09-07, raised by Marie). It hid at zero on the reasoning that an always-visible `0` trains the eye to skip the row. Sound in general, but it made this the **only** view in the browser that vanishes — *Recently deleted* and *Discarded by the system* are always-visible peers with reassuring empty states — and it cost two things. Its own empty state was **unreachable**, so nobody could meet the lane except at the moment it already had work in it. And `loadHeldCount` **reset `state.mode` at zero**, so settling the last record ejected the reader to the entries list at the instant they finished clearing the queue: peak trust, evidence removed, no confirmation.
> - **`0 held — N discarded memories examined`**, green, printed **at zero only**. `"0 held"` is a numerator, and alone it cannot separate *"the checker opened everything and found nothing"* from *"the checker never ran"*. `recordExamined` / `readExamined` (`reconcile-queue.ts`, `queue/examined.json`) keep the running total: **cumulative and on disk** (per-session resets return you to the ambiguous zero with extra steps), serialised through an in-process promise chain so concurrent turns cannot lose an increment, and **never throwing** — it runs inside turn ingestion under the same rule as the review pass itself. `reviewDiscardedEntries` counts **every row it opens**, not the subset it queues: counting only queued rows would make a working checker report `0 — 0` and look exactly like one that never ran. `0 — 0` on a fresh install is **correct output**, not a gap — it climbs on the first re-roll, so a stuck zero is evidence of itself.
>   - **The label names the verb only this lane performs.** Not "scored" — true in the retrieval lane, the turn log *and* the intake gate, which is exactly how a denominator from one population gets printed beside a numerator from another. And not "re-rolls examined" either: one re-roll fans out across two scopes, `discardLosingSwipe` returns *n* rows per scope, and a held record is appended **per row**. Rows are the population; re-rolls are a smaller, different one.
> - **`goMode(m)` is the ONLY place `state.mode` is assigned in `ui.ts`**, and a test asserts the occurrence count is exactly `1`. It scrolls to top on a **mode change only** — re-rendering in place after an edit must not throw a reader back up a long list — and `resolveHeldRecord` scrolls additionally when settling **empties** the lane. This came from rendering the empty state for the first time and *measuring* it: with a long sidebar the page sat at `scrollY 672` and "Nothing held" drew at `top: -599`, **off-screen**, because the lane's nav entry is at the **bottom** of the sidebar so the reader is always scrolled down on arrival. Reading the source did not find it; looking at it did. **Scroll position is part of "did the reader get the confirmation", not a cosmetic afterthought.**
> - **"Written" is not "seen."** `renderHeld`'s empty state existed for weeks and had **never once rendered**, because until `385b` it was unreachable. Marie's standing ruling, and it is a good one: treat an unreachable branch as *untested code with a comforting sentence in it* until somebody has laid eyes on it. It now reads — dashed card, green **Nothing held**, one muted ~56ch paragraph — and it is calm rather than alarming, which is what it was for.
