...
>
> **Read `references/architecture.md` first** for what actually replaced this surface: the **capability package** (`registerPromptContext`) injects memory while the Engine assembles the prompt, and the **poller** captures turns. The inference-proxy plan that this file used to point at (epic `hq7`) was **retired 2026-08-30** — do not follow it.
>
> This file is retained for two reasons only: to make the still-present `marinara-extender.js` in the repo intelligible, and as the **port checklist** for the panel rebuild (`ykz`) — the lanes, scope toggle, bookmarks, Import, Story ingest, Pending speakers, Identity, Recently deleted/Retired and Settings sections listed under "The panel UI" all have to reappear in a standalone page served by the sidecar. That second reason is now the important one: `ykz` is no longer gated behind anything, because the extension is already gone and its UI has **no replacement in shipped software**.
