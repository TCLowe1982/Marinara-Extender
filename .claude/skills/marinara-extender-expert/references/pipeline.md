...
`classifyAmbient` extracts durable identity/preference/history facts from throwaway lines. Facts have **no holding-pool lane**, so an unknown subject is **demoted to chat scope** rather than parked. Character-scope facts get `kind: "trait"` (the trait side of the dedup matrix vs. beats' `kind: "incident"`).

**Routing lives in ONE place: `resolveFactTarget` (`facts.ts`).** `api.ts` used to hold an inline copy, and `resolveFactTarget`'s own header described itself as "mirroring" it — which is not a safety property, it is a promise that two people will remember to edit both. They did not. `qlib` moved aboutness off the summary prose and into `subjects[]` in `facts.ts` while the live turn, the one path that runs on every message, went on writing `[about: X]` prefixes and no subject for another day across 1,552 entries (`oc4w`, 2026-08-25). Do not re-inline it.

**The `[about: X]` prefix is no longer writable at all.** `absorbAboutPrefix` (`subject.ts`) runs at `createEntry`'s mint point: a summary arriving with the prefix is stripped, `X` goes through `makeSubject` (which refuses a non-name and counts it), and the absorption is counted. Rung 3, not rung 5 — the entry is never refused, the claim just moves to the field where it belongs, so a re-inlined router shows up as a number instead of as archaeology later. One level of nesting is matched deliberately: `[about: [character]]` is 373 live entries and a naive `[^\]]*` welds a stray `]` onto the prose.

**MEASURED PRECISION, first time ever, 2026-08-25 (`qs67`):** 496 facts hand-labelled blind to arm, 60 real turns, `dolphin3:8b`. **43% precise — 92% SUPPORTED, 45% correctly ATTRIBUTED.** The model almost never fabricates; it gets the *subject* wrong more than half the time. Misattribution runs 15%. Read that before trusting anything this stage writes, and note that precision must be reported **per population** — a change that widens what the stage sees makes the old and new populations different questions, and one blended number hides the trade (blended, the widened arm scored 51% against 43% and looked like a clean win while its new population sat at 29%).

**THE CANDIDATE GATE, and the grammatical person it cannot see.** `extractCandidates` (`ambient.ts`) pre-filters every turn before the model is called: a sentence survives on length (10–120 chars), not ending in `?`, and matching `FIRST_PERSON_RE` (I/my/me/we/our) **or** `NAMED_SUBJECT_RE` (`^Name is|was|said|…`). There is no second person. So the gate admits a speaker describing *themselves* and a speaker naming a *third party*, and is structurally incapable of admitting a speaker describing **the person they are talking to** — the dominant register for facts about the user in a roleplay. Store-wide that discards 15,739 sentences, 92.7% of them directionally about the user, and it is the mechanism behind `su6h`'s omission half (Thomas's origin: stated 21 times, recorded zero).

`MARINARA_EXTENDER_SECOND_PERSON=1` admits them. **It defaults OFF because it failed its own pre-registered bar** (`cye6` slice 3): the population it admits measured 29% precise and 34% misattributed against a bar of ≥60% / ≤25%, and the pre-registered fallback (subject position rather than presence) scored 27% — *worse*, while dropping 62 of 161. The mechanism stays wired and tested; what is refused is turning it on before attribution is fixed.

**Direction of address is enforced in code for exactly one half.** `enforceAddressDirection` (`ambient.ts`): a `[user]`-block second-person-only sentence can never be about the user (one known speaker, so "not the speaker" names exactly who it is not) — reassigned to the session character, or stripped and demoted if there is no addressee to name. The `[character]` half is weaker: the block can carry several characters and the addressee is not knowable, so only the **speaker** is refused, the claim is **dropped rather than corrected**, and it is counted under the `speaker` rejection reason. Measured 85% precise (69 caught, 12 broken, 16 missed) — the 12 are speech acts where the character legitimately *is* the subject and are surface-identical to the 69, so reassigning them would mint false facts about the human, which is the failure the rule exists to stop.

**Gate it on second-person-ONLY, never on presence.** Restricted to the population the default gate admits, the presence form catches 2 misattributions and breaks 9 correct ones — 18%, net harmful. A "you" inside a first-person sentence is incidental and there the speaker really is the subject. Consequence: with the flag off the character half is inert by construction, so `qs67`'s code fix and `cye6` ship together or not at all.
...
**Belt and braces: there is now a real ordinal too** (`26d6e47`). `storage.ts` exposes `currentTurn`/`nextTurn`, a per-chat counter at `data/chats/<id>/turn-counter.yaml`, serialized on the file because the poller fires every 5s and can overlap itself. `process-turn` no longer defaults `turnNumber` to 0 — that default was the **same unset-vs-falsy conflation one layer up**, making "the caller sent nothing" indistinguishable from "the caller said turn zero". `/api/pre-turn` *reads* the counter without advancing; the ingest that follows owns the increment.

It is an **INGEST ORDINAL**, not the engine's message index: it counts what this sidecar ingested, a re-roll advances it, and every consumer needs only that it moves. The two fixes are deliberately independent — the sentinel keeps bookmarks correct if the counter is absent or wrong, the counter keeps the `% N` gates firing if a future mint forgets the sentinel.

**Pre-flight before landing it, because it makes tier promotion run for the first time.** The turn-gated `runPromotion` only re-tiers and archives to cold at 90 days (reversible — a recall miss brings them back); `deleteEntryFile` lives only in `runPromotionAll`, the manual backfill, and only for ghost entries with an empty summary. Measured blast radius on the live store: **24 of 12,202 hot entries, 0.2%** — that small *because of* `gwny`, since `lastAccessed` is refreshed just by being loaded, so the 90-day sweep can barely fire. Two bugs cancelling is not the same as either being fixed.
...
- **Never guess a subject into a permanent ledger.** Unknown subject → holding pool (beats) or chat scope with the name in `subjects[]` (facts). Guessing pollutes a character's memory irreversibly. The prose `[about: …]` prefix this used to say is dead: it is absorbed into the field at the mint point (`oc4w`) and cannot be written.
