...
**The detector** is `classifyChangelog` (`sentiment/changelog.ts`), **benched 2026-08-23, built but NOT WIRED.** `OPENER_FLOOR = 3` sentence-initial enumeration verbs (case-sensitive — "i added a note" is not a list item), `DIALOGUE_CEILING = 0.03` spares anything that is somebody talking. Returns the split, never a boolean: `reason` distinguishes `below-floor` (not a list) from `dialogue` (a list, but it's an ABOUT-WORK save worth counting separately).
