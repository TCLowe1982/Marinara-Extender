...
// atomicWriteFile_UNLOCKED_takeSerializedWriteYourself — the v1.1 durability floor (MarinaraExtender-1nc).
...
describe("atomicWriteFile_UNLOCKED_takeSerializedWriteYourself", () => {
  it("writes content and creates missing parent directories", async () => {
    const target = join(dir, "deep", "nested", "index.yaml");
    await atomicWriteFile_UNLOCKED_takeSerializedWriteYourself(target, "entries: []\n");
  ...
  it("replaces an existing file with the complete new content", async () => {
    ...
    await atomicWriteFile_UNLOCKED_takeSerializedWriteYourself(target, "version: 1\npayload: aaaaaaaaaaaaaaaaaaaa\n");
    await atomicWriteFile_UNLOCKED_takeSerializedWriteYourself(target, "version: 2\n");
  ...
  it("leaves no temp files behind after successful writes", async () => {
    ...
    for (let i = 0; i < 5; i++) await atomicWriteFile_UNLOCKED_takeSerializedWriteYourself(target, `n: ${i}\n`);
  ...
  it("survives concurrent writes to the same path — file is always one complete version", async () => {
    ...
    await Promise.all(versions.map((content) => atomicWriteFile_UNLOCKED_takeSerializedWriteYourself(target, content)));
